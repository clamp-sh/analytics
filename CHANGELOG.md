...
- [`c460054`](https://github.com/clamp-sh/clamp/commit/c460054aab73cc82d8d786435142d41e2a0bf767) Thanks [@sbj-o](https://github.com/sbj-o)! - Revenue tracking and session-level analytics.
  ...
  **MCP** — Three new tools: `get_revenue` (revenue split by currency, optionally grouped by any traffic dimension), `get_session_paths` (aggregate entry → exit paths with pages and duration per session), and `get_page_engagement` (per-page engagement seconds and bounce rate). Tool descriptions tightened to eliminate overlap between `get_breakdown` and the specialized breakdown tools.

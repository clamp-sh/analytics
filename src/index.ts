import type { EventPayload, BatchPayload, EventMap, AnyEvents, EventProperties } from "./types.js";
...
class BrowserClient<E extends EventMap = AnyEvents> {
  ...
  private installExtensions(ext?: Extensions) {
    if (!ext) return;
    const enabled = ext.outboundLinks || ext.downloads || ext.notFound || ext.dataAttributes || ext.webVitals || ext.sectionViews;
    if (!enabled) return;
    ...
    import("./extensions.js").then((mod) => {
      const track = (name: string, props?: EventProperties) => this.track(name as any, props as any);
